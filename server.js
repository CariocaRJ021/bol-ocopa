...
// Bancos de Dados em Memória Unificados
...
// Rotas de Autenticação e Lógica
app.post('/login', (req, res) => {
    ...
    if (USUARIOS_CADASTRADOS[user] && USUARIOS_CADASTRADOS[user] === req.body.password) {
        ...
        // Se ele veio por um link de convite, já jgrava a disputa certa na sessão
...
app.get('/', (req, res) => {
    ...
    // Captura o convite vindo pela URL (?convite=CÓDIGO)
    ...
    // Se o grupo atual for um personalizado, gera o link amigável para convidar amigos
    let htmlLinkConvite = '';
    if (dispAtual.id !== 'GLOBAL') {
        ...
        htmlLinkConvite = `
            <div style="background:#111827; border:1px solid #1f2937; padding:15px; margin-bottom:20px; border-radius:12px; display:flex; justify-content:space-between; align-items:center;">
                <span style="font-size:14px; color:#9ca3af;">✉️ Envie este link para convidar amigos para este grupo:</span>
                <input type="text" value="${linkCompleto}" readonly onclick="this.select(); document.execCommand('copy'); alert('Link de Convite copiado!');" style="width:340px; color:#f59e0b; text-align:center; font-weight:bold; cursor:pointer;">
            </div>`;
    }
